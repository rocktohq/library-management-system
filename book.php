...
require './includes/functions-book.php';
...
if(isset($_COOKIE['user'])) {
    ...
    if(isset($_POST['wishlist'])) {
        ...
        $sql = "SELECT * FROM `wishlist` WHERE `listed_by` = '$userid' AND `book_code` = '$book_code'";
        ...
        if($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO `wishlist` (book_code, listed_by) VALUES('$book_code', '$userid')";
            $result = $connect->query($sql);
            if($result) {
                $_SESSION['success'] = "Book has been Added to the WishList";
                header("Location: book.php?id={$book_id}");
            } else {
    ...
    if(isset($_POST['borrow'])) {
        ...
        $book_id = $_POST['book_id'];
         ...
         // Query to Feth the Boorows Details
        $sql = "SELECT * FROM `borrows` WHERE `borrowed_by` = '$userid' AND `returned` = 0";
        $result = $connect->query($sql);
        if($result->num_rows > 0) {
            $_SESSION['error'] = "You Already Borrowed a Book!";
            header("Location: book.php?id={$book_id}");
        } else {
            ...
            $result = $connect->query($sql);
            if($result) {
                $_SESSION['success'] = "Book is Listed, Wait for Confirmation";
                header("Location: book.php?id={$book_id}");
            } else {
                $_SESSION['error'] = "Error Adding Book to the List";
                header("Location: book.php?id={$book_id}");
            }
        }
    }
...
<head>
    ...
    <title><?php echo boookName($bookId); ?></title>
...
<body>
    ...
    <!-- ./BOOK  -->
    <section class="featured" id="book">
        <h1 class="heading"><span class="text-uppercase">Book Details</span></h1>

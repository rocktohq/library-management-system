...
if(isset($_COOKIE['lmsadmin'])) {
    ...
    if(isset($_GET['sid'])) {
        ...
        // Book Information
        $sql = "SELECT * FROM `admins` WHERE `id` = '$sid'";
        $result = $connect->query($sql);
        if(empty($row =  $result->fetch_assoc())) {
            echo "Invalid Librarian";
            exit();
        }
    ...
    if(isset($_POST['update'])) {
        ...
        if(empty($password = password_hash($_POST['password'], PASSWORD_DEFAULT))) {
        ...
        $sql = "SELECT EXISTS (SELECT * FROM `admins` WHERE `id` = '$sid') as `row_exists`  LIMIT 1";
        $result = $connect->query($sql);
        ...
        if($result->fetch_assoc()['row_exists']) {
            ...
            $result = $connect->query($sql);
            if($result) {
                $_SESSION['success'] = "Librarian Updated Successfully";
                header("Location: librarians.php");
            } else {
                $_SESSION['error'] = "Error Updating Librarian!";
                header("Location: updatelibrarian.php?sid={$sid}");
            }
        }
    }
...
<body>
    ...
    <main class="mt-5 pt-3">
        <div class="container">
            <div class="row">
                <div class="col-md-12 fw-bold fs-3">Update Librarian</div>
                <!-- ./NOTIFICATION -->
                <?php
                if(isset($success)) {
                ...
                <div class="row">
                    <div class="col-sm-8 offset-sm-2">
                        <form action="" method="post" enctype="multipart/form-data" class="row px-3 addstudent">
                            ...
                            <div class="mt-2">
                                <button name="update" type="submit" class="btn btn-primary text-uppercase">Update Librarian</button>
                            </div>
                        </form>
                    </div>
                </div>
